...
public class WorkerDAO implements DAO<Worker> {
    ...
    private final String QUERY_ORDER_BY = "SELECT * FROM trabajador ORDER BY trabajador.desde ASC";
    ...
    public Worker update(Worker worker) {
        ...
        try (PreparedStatement st = JDBCUtils.getConn().prepareStatement(UPDATE_BY_ID)) {
            ...
            if (rs == 1) {
                log.info("Actualización realizada correctamente");
                out = getWorkerByDNI(worker.getDni());
            }
